...
namespace BetterNotes.Framework
{
    ...
    /// </summary>
    public abstract class Notes_MBW : Notes_MBE
    {
        #region "Constructors"
        internal Notes_MBW()
            : base()
		...
		internal void DrawGUI()
		{
			...
			GUI.skin = Notes_SkinsLibrary.CurrentSkin;
        ...
        private void DrawToolTip()
        {
            ...
            if (TooltipsEnabled && Visible && (strToolTipText != "") && ((TooltipDisplayForSecs == 0) || (fltTooltipTime < (Single)TooltipDisplayForSecs)))
            {
                ...
                GUIStyle styleTooltip = Notes_SkinsLibrary.CurrentTooltip;
                ...
                if (TooltipMaxWidth > 0)
                {
                    ...
                    Notes_SkinsLibrary.CurrentTooltip.CalcMinMaxWidth(contTooltip, out minwidth, out maxwidth); // figure out how wide one line would be
                    _TooltipPosition.width = Math.Min(TooltipMaxWidth - Notes_SkinsLibrary.CurrentTooltip.padding.horizontal, maxwidth); //then work out the height with a max width
                    _TooltipPosition.height = Notes_SkinsLibrary.CurrentTooltip.CalcHeight(contTooltip, TooltipPosition.width); // heres the result
                }
                else
                {
                    //calc the new width and height
                    Vector2 Size = Notes_SkinsLibrary.CurrentTooltip.CalcSize(contTooltip);
