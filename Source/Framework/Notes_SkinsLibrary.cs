...
{
    internal static class Notes_SkinsLibrary
    {
        #region Constructor
        static Notes_SkinsLibrary()
        {
            _Initialized = false;
        ...
        {
            Notes_MBE.LogFormatted_DebugOnly("Setting GUISkin to {0}", DefaultSkin);
        ...
        {
            Notes_MBE.LogFormatted_DebugOnly("Setting GUISkin to {0}", SkinID);
            GUISkin OldSkin = _CurrentSkin;
            ...
            if (List.ContainsKey(SkinID))
                ...
                Notes_MBE.LogFormatted("Unable to change GUISkin to {0}, GUISkin not found", SkinID);
        ...
        internal static GUISkin CopySkin(DefSkinType DefaultSkin)
        {
            switch (DefaultSkin)
            {
                case DefSkinType.Unity: return (GUISkin)Notes_MBE.Instantiate(DefUnitySkin);
                case DefSkinType.KSP: return (GUISkin)Notes_MBE.Instantiate(DefKSPSkin);
                //case DefSkinType.None: return new GUISkin();
                default: return (GUISkin)Notes_MBE.Instantiate(DefKSPSkin);
            }
        }
        /// <summary>
        /// Copies a skin so you can make a new custom skin from an already defined one
        ...
        {
            if (List.ContainsKey(SkinID))
                return (GUISkin)Notes_MBE.Instantiate(List[SkinID]);
            else
            {
                Notes_MBE.LogFormatted("Unable to copy GUISkin to {0}, GUISkin not found", SkinID);
        ...
        internal static void AddStyle(ref GUISkin SkinToAction, GUIStyle NewStyle)
        {
            ...
            {
                Notes_MBE.LogFormatted("No Name Provided in the Style to add to {0}. Cannot add this.", SkinToAction.name);
                return;
            }
